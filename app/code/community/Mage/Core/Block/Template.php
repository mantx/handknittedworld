...
HTML;
        ...
        try {
            ...
            if (strpos($includeFilePath, realpath($this->_viewDir)) === 0 || $this->_getAllowSymlinks()) {
              if ($this->_isHamlTemplate($fileName)) {
                require_once 'PhamlP_3.2/haml/HamlParser.php';
                $hamlParser = new HamlParser();
                $xhtml = $hamlParser->haml2PHP($includeFilePath);
                if (eval(substr($xhtml, 5))===FALSE) {
                  Mage::log('Haml template syntax error:\n'.$xhtml, Zend_Log::ERR, null, true, true);
                }
              } else {
    ...
    protected function _isHamlTemplate($file) {
      $pos = strrpos($file, '.');
      return (substr($file, $pos) === '.haml') ;
    }
   ...
   public function renderChildView($template)
   {
     ...
     $this->setScriptPath(Mage::getBaseDir('design'));
     $templateName = Mage::getDesign()->getTemplateFilename($template, array('_relative'=>true));
